...
exports.load = async function (id) {
	try {
		console.log('[debug] load got', id);
		let { pathname } = new URL('file:///' + id);
		let tmp = require('url').pathToFileURL(id).href;
		console.log('[debug] load got', { id, pathname, tmp });
		let m = await Function('x', 'return import(x)')(id);
		console.log('[debug] here??', !!m);
		return m.default || m; // interop
	} catch (e) {
		console.log('[debug] load err', e);
		return require(id);
	}
}
